...
def groundsource_hp_example():
    ...
    data_path = os.path.join(
        os.path.dirname(__file__), "data/GSHP_example.csv"
    )
    ...
    energysystem.add(
        solph.components.Source(
            label="ambient", outputs={b_th_low: solph.Flow(nominal_value=30)}
        )
    )
    ...
    energysystem.add(
        solph.components.Sink(
            label="demand",
            inputs={
                b_th_high: solph.Flow(fix=data["demand_heat"], nominal_value=1)
            },
        )
    )
    ...
    energysystem.add(
        solph.components.Converter(
            ...
            outputs={
                b_th_high: solph.Flow(nominal_value=25, variable_costs=5)
            },
            conversion_factors={
