...
def airource_hp_variable_example():
    ...
    data_path = os.path.join(
        os.path.dirname(__file__), "data/ASHP_example.csv"
    )
    ...
    energysystem.add(
        solph.components.Source(
            label="el_grid", outputs={b_el: solph.Flow(variable_costs=10)}
        )
    )
    ...
    energysystem.add(
        solph.components.Sink(
            label="demand",
            inputs={
                b_heat: solph.Flow(fix=data["demand_heat"], nominal_value=1)
            },
        )
    )
    ...
    nominal_conditions = {"nominal_Q_hot": 25, "nominal_el_consumption": 7}

    max_Q_dot_heating = cmpr_hp_chiller.calc_max_Q_dot_heat(
        nominal_conditions, cops_ASHP
    )
    ...
    ASHP_output = string_results["ASHP", "heat"]["sequences"].values
    ...
    max_Q_dot_heating_abs = [
        nominal_conditions["nominal_Q_hot"] * max_heating
        for max_heating in max_Q_dot_heating
    ]
    ...
    print("********* Main results *********")
    print(
        "Total electricity consumption: {:2.1f}".format(
            ASHP_input.sum(axis=0)[0]
        )
    )
    print("Total heat output: {:2.1f}".format(ASHP_output.sum(axis=0)[0]))
    print(
    ...
    print(
        "Seasonal Performance Factor (SPF): {:2.2f}".format(
