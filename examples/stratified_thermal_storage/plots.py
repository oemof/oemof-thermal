"""
For this script to work as intended, please use oemof-solph v0.5.0 or higher.
In principle, it would run using v0.4, where the attributes
`fixed_losses_absolute` and `fixed_losses_relative` of the GenericStorage
were introduced, but the imports need to be changed.
"""
...
def plots():
    ...
    bus_simple_thermal_storage = Bus(
        label="bus_simple_thermal_storage", balanced=False
    )
    ...
    storage_list.append(
        GenericStorage(
            ...
            inputs={
                bus_simple_thermal_storage: Flow(
    ...
    for i, nominal_storage_capacity in enumerate([30, 65, 90]):
        ...
        storage_list.append(
            GenericStorage(
                ...
                outputs={
                    bus_i: Flow(
                        nominal_value=input_data[
                            "maximum_heat_flow_discharging"
                        ],
                        variable_costs=0.0001,
                    )
                },
    ...
    storage_content.columns = storage_content.columns.set_levels(
        [k.label for k in storage_content.columns.levels[0]], level=0
    )

    losses = -storage_content.iloc[1:, :].reset_index(
        drop=True
