...
def model_validation():
    ...
    def run_storage_model(initial_storage_level, temp_h, temp_c):
        ...
        heat_source = Source(
            label="heat_source",
            outputs={
                bus_heat: Flow(nominal_value=1, fix=heat_feedin_timeseries)
            },
        )
        ...
        optimization_model.write(
            "storage_model_facades.lp",
            io_options={"symbolic_solver_labels": True},
        )
        optimization_model.solve(solver="cbc", solve_kwargs={"tee": False})

        energysystem.results["main"] = solph.processing.results(
            optimization_model
        )
    ...
    plt.plot(
        TES_soc_relative_list[:end_step], label="storage level (simulation)"
    )
