...
def csp_collector_example():
    ...
    df_temp_amb_series = pd.read_csv(
        os.path.join(data_path, "data_Muscat_22_8_midday.csv")
    )
    df_temp_amb_series["Datum"] = pd.to_datetime(df_temp_amb_series["Datum"])
    df_temp_amb_series.set_index("Datum", inplace=True)
    df_temp_amb_series.index = df_temp_amb_series.index.tz_localize(
        tz="Asia/Muscat"
    )

    temp_amb_series = pd.read_csv(
        os.path.join(data_path, "temp_ambience.csv")
    )["t_amb"]
